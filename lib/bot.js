...
function handlePostback(userProfile, event) {
    switch (userProfile.conversationTopic) {
        ...
        case ConversationTopic.Browsing:
            handleBrowsingPostback(userProfile, event)
            break
...
function handleSexPreferencePostback(userProfile, event) {
    ...
    Messenger.sendMessage(userProfile.profileId, { text: `Noted.` }, (err) => {
        ...
        Messenger.sendMessage(userProfile.profileId, { text: `All set 🎉!` }, (err) => {
            ...
            Messenger.sendMessage(userProfile.profileId, { text: `Let's start looking...` }, (err) => {
                if (err)
                    return Messenger.reportSendErr(err)
...
function handleBrowsingPostback(userProfile, event) {
    const suggestionResult = event.postback.payload
    if (suggestionResult == 'like') {
        Messenger.sendMessage(userProfile.profileId, { text: `Awesome ❤!!!` }, (err) => {
            if (err)
                return Messenger.reportSendErr(err)
        })
    } 
    else if (suggestionResult == 'next') {
        Messenger.sendMessage(userProfile.profileId, { text: `Bummer.` }, (err) => {
            if (err)
                return Messenger.reportSendErr(err)
